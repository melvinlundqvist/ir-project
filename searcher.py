...
def format_preferences_search(user_name, results):
    top_k_to_save = 5
    max_list_length = 10
    if len(results) < top_k_to_save:
        top_k_to_save = len(results)
    for i in range(top_k_to_save):
        # read user file
        ...
        with open("Users.json", "r") as jsonFile:
            ...
            for user in users:
                if user['name'] == user_name:
                    ...
                    if len(user['history']) > max_list_length:
                        user['history'].pop(0)
    ...
    with open("Users.json", "r") as jsonFile:
        ...
        for user in users:
            if user['name'] == user_name:
                ...
                if len(user['click']) > max_list_length:
                    user['click'].pop(0)
